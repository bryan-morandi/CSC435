...
public class JokeClientAdmin {
    public static void main(String[] args) {
        String serveName;
        System.out.println("Bryan Morandi's Joke Client Admin.\n");
        ...
        int primaryPortNum = 5050;
        int secondaryPortNum = 5051;
        ...
        try {
            ...
            do {
                ...
                quitString = in.readLine(); //reads users input from the terminal

                if (!quitString.contains("quit"))  //if user hasn't entered 'quit' into the terminal, then find the remote address
                    if (quitString.contains("s")) {
                        ...
                        toggleMode(JokeServer.serverMode, currentServer, currentPort);
                    }
            } while (!quitString.contains("quit")); //while user has not entered 'quit' in terminal window
            System.out.println("Cancelled by user request");
        } catch (IOException ex) {ex.printStackTrace();}
    }

    static void toggleMode(Boolean mode, String serveName, int serverPort){
        Socket socket;
        PrintStream toServe;
        ...
        try {
            ...
            socket = new Socket(serveName, serverPort);
            ...
            if(mode) {
                ...
                modeToStitch = "Proverb";
            }

            toServe.println(modeToStitch); //sends the mode that needs to be switched

            textFromServe = fromServe.readLine(); //reads the mode that was sent by Joke Server
